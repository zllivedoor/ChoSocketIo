...
void HelloWorld::nodeServerCon()
{
  ...
  _sioClient->on("battleBreak", CC_CALLBACK_2(HelloWorld::battleBreakEvent, this));
...
void HelloWorld::roomAJoinCallback(cocos2d::Object *sender,TouchEventType type)
{
  ...
  if (type == TOUCH_EVENT_ENDED)
  {
    ...
    // Join Room画面を非表示する
    auto loginLayer = (UILayer*)getChildByTag( 99999);
...
void HelloWorld::battleBreakEvent(SIOClient *client, const std::string& data) {

  //cocos2d-xでJSONを使う http://nirasan.hatenablog.com/entry/2013/10/24/232905
	log("Att OVER【battleBreakEvent】: %s", data.c_str());
  picojson::object obj = HelloWorld::getArgs(data);
...
log("1000pos %i",5000 + pos);
  ...
  if (pt){
    ...
    pt->runAction(CCSequence::create(DelayTime::create(0.4),
                                     CCShaky3D::create(0.3, Size(20,20), 5, false),
                                     CCFadeOutTRTiles::create(1, Size(100,100)) ,
                                     CCRemoveSelf::create(true),
                                     NULL));
  }
}
